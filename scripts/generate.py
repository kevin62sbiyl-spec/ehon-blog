...
def bulk_generate(target_category: str = None):
    ...
    # 最後にトップページとカテゴリページを再ビルド
    all_posts = load_all_posts()
    (DOCS_DIR / "index.html").write_text(generate_index_html(all_posts), encoding="utf-8")
    auto_build_categories(all_posts)
...
# ── カテゴリページ自動ビルド ──────────────────────────────────
def auto_build_categories(all_posts: list):
    """記事生成後に自動でカテゴリページを更新する"""
    CATEGORY_DESC = {
    ...
    cat_nav = "".join([
        ...
        f'<span>{cat}</span><span class="cat-nav-count">{len(posts)}本</span></a>'
    ...
    for cat, posts in by_cat.items():
        ...
        cards = "".join([
            f'<a class="post-card" href="{site_url}/posts/{p["slug"]}.html">'
            f'<div class="card-meta"><span class="category-badge">{cat}</span><time>{p.get("date","")}</time></div>'
        ...
        sections_html += f'''<section class="cat-section" id="{cat}">
  <div class="cat-header">
    <span class="cat-icon">{icon}</span>
    <div><h2 class="cat-title">{cat}</h2><p class="cat-desc">{desc}</p></div>
...
<header class="site-header">
  <div class="container">
    <a class="logo" href="index.html">{SITE_CONFIG['author']}</a>
    <nav><a href="index.html">ホーム</a><a href="categories.html">カテゴリ</a><a href="{yt_ch}" target="_blank" rel="noopener">YouTube</a></nav>
  </div>
</header>
...
<script src="assets/js/main.js"></script>
    ...
    (DOCS_DIR / "categories.html").write_text(html, encoding="utf-8")
    print(f"  ✅ カテゴリページ更新（{len(all_posts)}記事）")
